# RStudio and Positron installers for Manjaro/Arch Linux

[![BuyMeACoffee](https://raw.githubusercontent.com/pachadotdev/buymeacoffee-badges/main/bmc-yellow.svg)](https://www.buymeacoffee.com/pacha)

# Installation

RStudio

```bash
wget https://github.com/pachadotdev/rstudio-positron-manjaro/releases/download/2025-09-07/rstudio-desktop-2025.05.1-1-x86_64.pkg.tar.zst
sudo pacman -U rstudio-desktop-2025.05.1-1-x86_64.pkg.tar.zst
```

Positron

```bash
wget https://github.com/pachadotdev/rstudio-positron-manjaro/releases/download/2025-09-07/positron-2025.09.0-6-x86_64.pkg.tar.zst
sudo pacman -U positron-2025.09.0-6-x86_64.pkg.tar.zst
```

See the blog post here: https://pacha.dev/blog/2025/09/07/rstudio-positron-manjaro/
